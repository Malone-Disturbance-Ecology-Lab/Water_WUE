...
exceeds observed ET) occurred in the final analytical dataset.
...
all_events = []  # list of dicts to store individual exceedance values
site_summaries = []
...
for site in final_domain['site_name'].unique():
    ...
    if dt_candidates:
        ...
        print(f"  {site}: using datetime column '{dt_col}'")
        df_site['DateTime'] = pd.to_datetime(df_site[dt_col], errors='coerce')
    else:
        # Try to construct from Year, month, day, Hour
        if all(c in df_site.columns for c in ['Year', 'month', 'day', 'Hour']):
            print(f"  {site}: constructing DateTime from Year, month, day, Hour")
