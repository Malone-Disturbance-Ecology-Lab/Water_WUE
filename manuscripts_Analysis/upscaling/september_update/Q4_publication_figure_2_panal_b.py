# -*- coding: utf-8 -*-
"""
Panel B – Annual impacted-area time series
- Four stacked subplots (one per coast)
- Shows SPEI-3 dry area (gold shading), positive event area, and
  negative event area. The dashed "any signed event area" line has been
  removed because it is derived from the other two and did not add a
  distinct pattern.
...
for ax, region in zip(axes, REGIONS):
    ...
    ax.fill_between(years, 0, dry_area, color=DRY_AREA_COLOR, alpha=0.32,
                    label="Dry area")
    ax.plot(years, neg_area, color=NEGATIVE_EVENT_COLOR, marker="o",
            markersize=2.2, linewidth=1.2,
            label="Negative event area")
    ax.plot(years, pos_area, color=POSITIVE_EVENT_COLOR, marker="o",
            markersize=2.2, linewidth=1.2,
            label="Positive event area")

    ylim = percent_axis_limit(neg_area, pos_area, dry_area)
    ax.set_ylim(0, ylim)
...
# Shared legend (above the panels) – now three items
