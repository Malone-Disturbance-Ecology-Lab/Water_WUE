...
reddy_proc <- function(file_path, save_path, files) {
  for (full_file_path in files) {
    tryCatch({
      ...
      required_columns <- c('NEE', 'LE', 'H', 'Rg', 'Tair', 'VPD', 'Ustar', 'PA', 'NETRAD')
      ...
      # loop is for if user defined ustar is used
      ...
      # Define mapping from filled2 column names to output names
      optional_cols <- list(
        NEE_uStar_f    = "NEE_f",
        LE_uStar_f     = "LE_f",
        Tair_f         = "Tair_f",
      ...
      # Add columns to filled3 if they exist in filled2
      ...
      print(paste("Standard offset:", standard_offset))  # <- should print this now
    }, error = function(e) {
