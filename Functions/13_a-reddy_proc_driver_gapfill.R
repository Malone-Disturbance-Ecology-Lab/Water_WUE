  ...
  tryCatch({
    ...
    if (nrow(site_info_specific) > 0) {
      ...
      if (is.na(lat) || is.na(lon)) {
        stop("Invalid coordinates for site ", SITE_ID, ": Lat = ", lat, ", Lon = ", lon)
      }
    }
...
process_all_files <- function(file_path, save_path) {
  # Get all CSV files
  csv_files <- list.files(file_path, pattern = "\\.csv$", full.names = TRUE)
  
  if (length(csv_files) == 0) {
    message("No CSV files found in: ", file_path)
    return()
  }
  
  message("Found ", length(csv_files), " CSV files to process")
  
  # Create a temporary directory for local copies
  ...
  for (i in seq_along(csv_files)) {
    full_file_path <- csv_files[i]
    file_name <- basename(full_file_path)
    
    message("\n", "="^50)
    message("Processing file ", i, " of ", length(csv_files), ": ", file_name)
    message("="^50)
    
    # Create local copy to avoid permission issues
    local_file_path <- file.path(local_temp_dir, file_name)
    
    file.copy_result <- tryCatch({
      file.copy(full_file_path, local_file_path)
    }, error = function(e) {
      message("Failed to create local copy: ", e$message)
      FALSE
    })
    
    if (file.copy_result) {
      # Process the local copy
      success <- reddy_proc_single(local_file_path, save_path)
      ...
      if (success) {
        ...
        message("✗ Failed to process: ", file_name)
      }
    } else {
      failed_count <- failed_count + 1
      failed_files <- c(failed_files, file_name)
      message("✗ Failed to create local copy: ", file_name)
    }
  }
  ...
  message("="^50)
  message("Total files processed: ", length(csv_files))
...
# Run the processing
process_all_files(file_path, save_path)