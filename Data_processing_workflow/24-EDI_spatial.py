...
------
  Masked SPEI-3 files:
    <MALONE_ROOT>/Research/WUE_CUE/spatial_SPEI/SPEI-3/
    SPEI3_*_US_OCEAN_COAST_80km.nc

Outputs
-------
  <MALONE_ROOT>/Research/WUE_CUE/spatial_SPEI/EDI_spatial/
...
# ---------------------------------------------------------------------------
# PATHS  — adjust MALONE_ROOT if needed
# ---------------------------------------------------------------------------
MALONE_ROOT = os.environ.get("MALONE_ROOT", "/Volumes/MaloneLab")          # macOS Finder mount
SPEI3_DIR   = os.environ.get("SPEI3_DIR", f"{MALONE_ROOT}/Research/WUE_CUE/spatial_SPEI/SPEI-3")
OUTPUT_DIR  = os.environ.get("EDI_OUTPUT_DIR", f"{MALONE_ROOT}/Research/WUE_CUE/spatial_SPEI/EDI_spatial")
...
RESPONSE_CURVES = (
    PROJECT_ROOT / "Malone_Workflow" / "results" / "Ecological_Impacts" /
...
Q3_SLOPE_SUMMARY = (
    PROJECT_ROOT / "Malone_Workflow" / "results" /
    "Q3_WUE_T_SPEI_sensitivity" /
    "Q3_WUE_T_SPEI_coast_region_slope_summary.csv"
)
Q3_THRESHOLD_SUMMARY = (
    PROJECT_ROOT / "Malone_Workflow" / "results" /
    "Q3_WUE_T_SPEI_sensitivity" /
    "Q3_WUE_T_SPEI_coast_predicted_threshold_summary_5_10_20pct.csv"
)
Q4_DECLINE_THRESHOLDS = (
    PROJECT_ROOT / "Malone_Workflow" / "results" /
    "Q4_WUE_decline_SPEI" /
    "Q4_WUE_decline_5pct_probability_thresholds_by_group.csv"
)
...
def main():
    ...
    if not Path(MALONE_ROOT).exists():
        sys.exit(
            f"MaloneLab mount not found at {MALONE_ROOT}. "
            "Mount MaloneLab, then rerun this script."
        )
    ...
    print(
        ...
        f"for {SPATIAL_START_YEAR}-{SPATIAL_END_YEAR}"
    )
